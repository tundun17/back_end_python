...
class AutomationView(APIView):
    ...
    def get(self, request):
        ...
        return Response(
            [automation_rule_to_dict(rule) for rule in rules],
            status=status.HTTP_200_OK,
        )
    ...
    def post(self, request):
        ...
        if alert_type not in Alert.AlertType.values:
            return Response(
                {"alert_type": "alert_type khong hop le."},
                status=status.HTTP_400_BAD_REQUEST,
            )

        if active_state not in Switch.State.values or normal_state not in Switch.State.values:
            return Response(
                {"state": "active_state va normal_state phai la ON hoac OFF."},
                status=status.HTTP_400_BAD_REQUEST,
            )

        sensor_esp = get_object_or_404(
            ESP,
            hashcode=sensor_hashcode,
            home__owner=request.user,
        )
        if not sensor_esp.is_sensor:
            return Response(
                {"sensor_hashcode": "ESP nay khong phai sensor."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if target_switch.esp_device.home_id != sensor_esp.home_id:
            return Response(
                {"message": "Sensor ESP va switch dieu khien phai thuoc cung mot nha."},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class AutomationDetailView(APIView):
    ...
    def get(self, request, id):
        ...
        return Response(automation_rule_to_dict(rule), status=status.HTTP_200_OK)
    ...
    def patch(self, request, id):
        ...
        if "active_state" in request.data:
            if request.data["active_state"] not in Switch.State.values:
                return Response(
                    {"active_state": "active_state phai la ON hoac OFF."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        ...
        if "normal_state" in request.data:
            if request.data["normal_state"] not in Switch.State.values:
                return Response(
                    {"normal_state": "normal_state phai la ON hoac OFF."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        ...
        return Response(
            {
                "message": "Cap nhat automation rule thanh cong.",
                "automation_rule": automation_rule_to_dict(rule),
            },
            status=status.HTTP_200_OK,
        )
    ...
    def delete(self, request, id):
        ...
        return Response(
            {"message": "Xoa automation rule thanh cong."},
            status=status.HTTP_200_OK,
        )
